...
- 2025-09-12: Firestore向けストレージプロバイダを追加し、`tenants/{tenant_id}/sessions/{session_id}` でデータ保存を実装。
- 2025-09-13: Cloud Run用ディレクトリと`cloudrun.yaml`を追加し、`deploy-cloudrun`ターゲットとデプロイ手順を整備。
- 2025-09-14: レガシーな事前アドバイスページ関数を削除。
- 2025-09-15: モバイル向けサイドバートグルにテキストラベルと翻訳キーを追加。
- 2025-09-16: ストレージサービスで環境変数を取得するため`os`をインポートするよう修正。
